  ...
  return (
    <GlassCard edge className="flex h-full flex-col p-5">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h3 className="font-display text-base font-semibold text-white">{t("pos.currentSale")}</h3>
        {lines.length > 0 && (
      ...
      <div className="flex-1 space-y-2 overflow-y-auto pr-1 scrollbar-none">
        {lines.length === 0 ? (
          <div className="flex h-full min-h-[200px] flex-col items-center justify-center gap-2 text-center">
            ...
            <p className="text-sm text-ink-100/40">{t("pos.cartEmpty")}</p>
          </div>
        ) : (
          lines.map((line) => (
              ...
              <div className="mt-2 flex items-center justify-between">
                <div className="flex items-center gap-1 rounded-lg bg-white/5 p-1">
                  <Tooltip content="Decrease quantity">
                    <button
                      data-testid={`cart-qty-decrease-${line.batch_id}`}
                  ...
                  <span data-testid={`cart-qty-${line.batch_id}`} className="w-6 text-center text-sm font-medium text-white">{line.quantity}</span>
                  <Tooltip content={`Increase quantity (max ${line.available_qty} in stock)`}>
                    <button
                      data-testid={`cart-qty-increase-${line.batch_id}`}
      ...
      <div className="mt-4 space-y-4 border-t border-white/10 pt-4">
        <div className="flex items-center justify-between">
          <span className="text-sm text-ink-100/60">{t("common.total")}</span>
          <span className="font-display text-2xl font-semibold text-white">{formatCurrency(subtotal)}</span>
        </div>

        <div>
          <p className="mb-2 text-xs font-medium text-ink-100/50">{t("pos.paymentMethod")}</p>
        ...
        >
          {t("pos.completeSale")} · {formatCurrency(subtotal)}
        </Button>
      </div>
    </GlassCard>
  );
